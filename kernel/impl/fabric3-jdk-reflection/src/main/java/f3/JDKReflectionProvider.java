...
public class JDKReflectionProvider {
    ...
    private static final QName QNAME = new QName(Namespaces.F3, "JDKReflectionComposite");

    @Provides
    public static Composite getComposite() {
        CompositeBuilder compositeBuilder = CompositeBuilder.newBuilder(QNAME);

        compositeBuilder.component(newBuilder(JDKInstantiatorFactory.class).build());
        compositeBuilder.component(newBuilder(JDKInjectorFactory.class).build());
        compositeBuilder.component(newBuilder(JDKLifecycleInvokerFactory.class).build());
        compositeBuilder.component(newBuilder(JDKServiceInvokerFactory.class).build());
        compositeBuilder.component(newBuilder(JDKConsumerInvokerFactory.class).build());
