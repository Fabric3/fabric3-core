...
public class TomcatWebApplicationActivator implements WebApplicationActivator {
	...
	public ServletContext activate(String contextPath, URI uri,
			URI parentClassLoaderId, Map<String, List<Injector<?>>> injectors,
			ComponentContext componentContext)
			throws WebApplicationActivationException {
		if (mappings.containsKey(uri)) {
			throw new WebApplicationActivationException(
					"Mapping already exists: " + uri.toString());
		}
		...
		try {
			...
			ClassLoader parentClassLoader = createParentClassLoader(
					parentClassLoaderId, uri);
			StandardContext context = createContext(contextPath,
					resolved.getFile(), parentClassLoader, injectors);
			for (Container container : connector.getService().getContainer()
					.findChildren()) {
				if (container instanceof StandardHost) {
					container.addChild(context);
				}
			}
			// Setup the session listener to inject reference proxies in newly
			// created sessions.
			// Note this must be performed after the context is added as a child
			// to StandardHost as doing so
			// starts the context which results in the application lifecycle
			// listeners being reset.
			List<Injector<HttpSession>> sessionInjectors = List.class.cast(injectors.get(SESSION_CONTEXT_SITE));
			InjectingSessionListener listener = new InjectingSessionListener(
					sessionInjectors);
	...
	private ClassLoader createParentClassLoader(URI parentClassLoaderId, URI id) {
		ClassLoader cl = classLoaderRegistry
				.getClassLoader(parentClassLoaderId);
		return new MultiParentClassLoader(id, cl);
	}

	private StandardContext createContext(String path, String docBase,
			ClassLoader classLoader, Map<String, List<Injector<?>>> injectors) {
		...
		// turn off unpacking so Tomcat does not copy the extracted WAR to the
		// /webapps directory and autodeploy it on a subsequent boot
		context.setUnpackWAR(false);
		Fabric3InstanceManager instanceManager = new Fabric3InstanceManager(
				injectors, classLoader);
		context.setInstanceManager(instanceManager);
		context.getServletContext().setAttribute(
				InstanceManager.class.getName(), instanceManager);
		return (context);
	}

	@SuppressWarnings({ "unchecked" })
	private void injectServletContext(ServletContext servletContext,
